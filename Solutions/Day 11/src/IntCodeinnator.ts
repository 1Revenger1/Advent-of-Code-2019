...
export class IntCodeinnator {
    ...
    constructor(image : Storage) {
        ...
        this.ops.set(4,  { length: 2, pointerMask: 0,               run: async (inst, memory) => { if(this.outputFunction) { await this.outputFunction(inst.noun)}; this.output.push(inst.noun) }}); // Print
